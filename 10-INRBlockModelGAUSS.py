...
#Gauss layer, from https://github.com/liuzhen0212/FINER/blob/main/models.py
class GaussLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=True, scale=5.0):
        super().__init__()
...
class DensityContrast(nn.Module):
    def __init__(self, hidden=128, depth=3, scale=5.0, rho_abs_max=450.0):
        super().__init__()
        self.rho_abs_max = rho_abs_max

        layers = []

        #First Gauss layer takes x,y,z
        layers.append(GaussLayer(3, hidden, scale=scale))

        #Stack of hidden layers
        for _ in range(depth - 1):
            layers.append(GaussLayer(hidden, hidden, scale=scale))
        ...
        #Init all layers
        self._init_gaussian_layers(scale)

    def _init_gaussian_layers(self, scale):
        with torch.no_grad():
            for m in self.modules():
                if isinstance(m, GaussLayer):
                    fan_in = m.linear.weight.shape[1]
                    std = 1.0 / (scale * (fan_in ** 0.5))
                    nn.init.normal_(m.linear.weight, mean=0.0, std=std)
                    nn.init.uniform_(m.linear.bias, -0.5, 0.5)
...
def train_inr(model, opt, coords_norm, G, gz_obs, Wd, Nx, Ny, Nz, dx, dy, dz, cfg):
    ...
    for ep in range(cfg['epochs']):
        ...
        smooth_term = smoothness_penalty(m_pred, Nx, Ny, Nz, alpha=cfg.get('alpha', 1e-3))
        loss = data_term + smooth_term
...
#Smoothness penalty to stabilize model
def smoothness_penalty(m_pred, Nx, Ny, Nz, alpha=1e-3):
    m3d = m_pred.view(Nx, Ny, Nz)
    dx = (m3d[1:, :, :] - m3d[:-1, :, :]).pow(2).mean()
    dy = (m3d[:, 1:, :] - m3d[:, :-1, :]).pow(2).mean()
    dz = (m3d[:, :, 1:] - m3d[:, :, :-1]).pow(2).mean()
    return alpha * (dx + dy + dz)
