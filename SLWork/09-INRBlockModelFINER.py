...
#From https://github.com/liuzhen0212/FINER
class FinerLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=True, is_first=False, omega_0=20.0,
                 first_bias_scale=0.1, scale_req_grad=False, max_scale=3.0, scale_mode="tanh"):
        super().__init__()
        ...
        self.init_weights()
        if self.first_bias_scale is not None and self.linear.bias is not None:
    ...
    def init_weights(self):
        with torch.no_grad():
            if self.is_first:
                ...
                self.linear.weight.uniform_(-np.sqrt(6/self.in_features)/max(self.omega_0, 1e-6),
                                             np.sqrt(6/self.in_features)/max(self.omega_0, 1e-6))
    ...
    def generate_scale(self, x):
        ...
        if self.scale_mode == "tanh":
            if self.scale_req_grad:
                scale = 1.0 + self.max_scale * torch.tanh(base / self.max_scale)
            else:
                with torch.no_grad():
                    scale = 1.0 + self.max_scale * torch.tanh(base / self.max_scale)
        else:
            if self.scale_req_grad:
                scale = 1.0 + torch.clamp(base, 0.0, self.max_scale)
            else:
                with torch.no_grad():
                    scale = 1.0 + torch.clamp(base, 0.0, self.max_scale)
        return scale
...
class Finer(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers, out_features,
                 first_omega_0=20.0, hidden_omega_0=5.0, bias=True,
                 first_bias_scale=0.1, scale_req_grad=False, max_scale=3.0, scale_mode="tanh"):
        super().__init__()
        net = []
        net.append(FinerLayer(in_features, hidden_features, bias=bias, is_first=True,
                              omega_0=first_omega_0, first_bias_scale=first_bias_scale,
                              scale_req_grad=scale_req_grad, max_scale=max_scale, scale_mode=scale_mode))
        for _ in range(hidden_layers):
            net.append(FinerLayer(hidden_features, hidden_features, bias=bias, is_first=False,
                                  omega_0=hidden_omega_0, scale_req_grad=scale_req_grad,
                                  max_scale=max_scale, scale_mode=scale_mode))
        final_linear = nn.Linear(hidden_features, out_features)
        with torch.no_grad():
            final_linear.weight.uniform_(-np.sqrt(6/hidden_features)/max(hidden_omega_0, 1e-6),
                                          np.sqrt(6/hidden_features)/max(hidden_omega_0, 1e-6))
...
class DensityContrastINR(nn.Module):
    def __init__(self, hidden=256, depth=4, rho_abs_max=600.0,
                 first_omega_0=20.0, hidden_omega_0=5.0,
                 first_bias_scale=0.1, scale_req_grad=False,
                 max_scale=3.0, scale_mode="tanh"):
        super().__init__()
        in_dim = 3
...
def run():
    ...
    cfg = dict(gamma=1.0, epochs=300, lr=5e-3) #lr=1e-2
